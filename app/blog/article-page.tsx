...
export default function ArticlePage({ post }: { post: BlogPost }) {
  return <main className="article-page">
    <header className="article-header"><a className="official-logo" href="/" aria-label="العودة إلى ترتيب"><img src="/tarteeb-logo-official.png" alt="شعار ترتيب"/></a><a href="/#blog">العودة إلى المدونة ←</a></header>
    <article>
      <div className="article-hero"><div><p className="eyebrow">{post.category} · {post.readTime}</p><h1>{post.title}</h1><p>{post.excerpt}</p></div><img src={post.image} alt={post.title}/></div>
