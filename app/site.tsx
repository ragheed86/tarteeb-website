...
export default function TarteeebSite(){
  ...
  return <main>
    ...
    <section id="blog" className="section blog-section"><div className="section-head"><div><p className="eyebrow">مدونة ترتيب</p><h2>أفكار عملية لمساحة تستمر.</h2></div><p>قصص حقيقية وإرشادات بسيطة تساعدك على بناء نظام يناسب يومك.</p></div><div className="blog-grid">{blogPosts.map((post)=><article key={post.slug}><a className="blog-image" href={`/blog/${post.slug}`}><img src={post.image} alt={post.title} loading="lazy"/></a><div><span>{post.category} · {post.readTime}</span><h3><a href={`/blog/${post.slug}`}>{post.title}</a></h3><p>{post.excerpt}</p><a className="blog-link" href={`/blog/${post.slug}`}>اقرئي المقال ←</a></div></article>)}</div></section>
