...
export default function TarteeebSite(){
  ...
  return <main>
    <header><a className="official-logo" href="#top" aria-label="ترتيب Arrange and Organize"><img src="/tarteeb-logo-official.png" alt="شعار ترتيب الرسمي"/></a><button className="menu" aria-label="فتح القائمة" onClick={()=>setMenu(!menu)}>☰</button><nav className={menu?"open":""}>{[["قبل وبعد","top"],["خدماتنا","services"],["منهج ترتيب","method"],["من نحن","about"],["الأسئلة الشائعة","faq"]].map(([l,id])=><a key={id} href={`#${id}`} onClick={()=>setMenu(false)}>{l}</a>)}</nav><a className="header-cta" href="https://wa.me/?text=مرحبًا،%20أرغب%20في%20تقييم%20مساحتي" target="_blank">أرسلي صور المساحة</a></header>
    <section id="top" className="comparison-hero"><div className="comparison-intro"><p className="eyebrow">تنظيم المنازل والمكاتب في الرياض</p><h1>اسحبي لرؤية الفرق.</h1><p>من الفوضى إلى نظام واضح يناسب يومك.</p><div className="actions"><a className="primary" href="https://wa.me/?text=مرحبًا،%20أرغب%20في%20تقييم%20مساحتي" target="_blank">أرسلي صور المساحة</a><a href="#services">استكشفي الخدمات</a></div></div><BeforeAfter label="المطبخ" before="/images/kitchen-before.jpg" after="/images/kitchen-organized.jpg"/></section>
    ...
    <section id="services" className="section services-section"><div className="section-head"><div><p className="eyebrow">خدماتنا</p><h2>نرتب المساحة حول حياتك.</h2></div><p>مرّري المؤشر على الصورة لرؤية النتيجة.</p></div><div className="service-grid">{services.map(([n,d,before,after])=><article className="service" key={n} tabIndex={0}><div className="service-media"><img className="service-before" src={before} alt={`${n} قبل التنظيم`} loading="lazy"/><img className="service-after" src={after} alt={`${n} بعد التنظيم`} loading="lazy"/><span className="service-state before-state">قبل</span><span className="service-state after-state">بعد</span></div><div><h3>{n}</h3><p>{d}</p><a href="#assessment">اطلبي تقييمها ←</a></div></article>)}</div></section>
    ...
    <section className="section journey journey-graphic"><div className="journey-title"><p className="eyebrow">كيف تبدأ الخدمة؟</p><h2>أربع خطوات فقط.</h2><p>من الصورة الأولى إلى مساحة جاهزة.</p></div><div className="journey-canvas">{[[PiImagesLight,"أرسلي الصور","صور أو فيديو للمساحة"],[PiMagnifyingGlassLight,"نراجع المساحة","نحدد الاحتياج والنطاق"],[PiFileTextLight,"يصلك العرض","سعر واضح وموعد مناسب"],[PiCheckCircleLight,"ننفذ ونسلّم","نظام جاهز للاستخدام"]].map(([Icon,title,text],i)=><article key={title as string}><div className="journey-icon"><Icon aria-hidden="true"/></div><span className="journey-number">0{i+1}</span><h3>{title as string}</h3><p>{text as string}</p></article>)}</div><a className="journey-action" href="https://wa.me/?text=مرحبًا،%20أرغب%20في%20تقييم%20مساحتي" target="_blank">ابدئي بإرسال الصور</a></section>
    ...
    <section id="faq" className="section faq"><p className="eyebrow">قبل أن تبدئي</p><h2>أسئلة شائعة</h2><div>{faqs.map(([q,a])=><details key={q}><summary>{q}<span>+</span></summary><p>{a}</p></details>)}</div></section>
